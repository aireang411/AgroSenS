...
type ReglaVariable = {
    clave: 'temperatura' | 'humedad' | 'dpv';
    tipo_riesgo: 'pre_critico' | 'critico';
    valor: number;
    mensaje: string;
 ...
 */
async function generarAlerta(req: CustomRequest, res: Response): Promise<void> {
 ...
 * GET /api/v1/alertas
 */
async function obtenerAlertas(req: CustomRequest, res: Response): Promise<void> {
    const idUsuario = req.usuario?.id_usuario;
    ...
    try {
        ...
        const params: any[] = [idUsuario, dias];
 ...
 */
async function obtenerAlerta(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    const idUsuario = req.usuario?.id_usuario;
 ...
 */
async function actualizarAlerta(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    const idUsuario = req.usuario?.id_usuario;
 ...
 */
async function marcarAlertaVista(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    const idUsuario = req.usuario?.id_usuario;
...
/**
 * Obtener y generar alertas/recomendaciones por cultivo seleccionado
 * GET /api/v1/alertas/cultivo/:id_lote
 */
async function obtenerAlertasPorCultivo(req: CustomRequest, res: Response): Promise<void> {
    ...
    try {
        ...
        if (!lote) {
            res.status(404).json({ error: 'Cultivo no encontrado', code: 'LOTE_NOT_FOUND' });
            return;
        }
        ...
        if (lectura) {
            ...
            if (temperatura >= 36) {
                reglas.push({
                    ...
                    tipo_riesgo: 'critico',
                    valor: temperatura,
                    mensaje: `Temperatura alta (${temperatura.toFixed(1)} °C). El cultivo puede entrar en estrés térmico.`,
                    recomendacion: 'Hace bastante calor en este cultivo. Te conviene ventilar y revisar riego corto para bajar estrés.',
                    acciones: [
                        'Aumentar ventilación o abrir laterales del invernadero',
                        'Aplicar riego corto de enfriamiento en horas de mayor calor',
                ...
                reglas.push({
                    ...
                    valor: temperatura,
                    mensaje: `Temperatura en nivel medio (${temperatura.toFixed(1)} °C). Conviene corregir antes de que suba más.`,
                    recomendacion: 'La temperatura se está saliendo del rango cómodo. Con algo más de ventilación lo puedes estabilizar.',
                    acciones: [
                        'Ventilar en las horas más cálidas',
                        'Revisar si hay acumulación de calor en zonas cerradas',
                        'Monitorear de nuevo en 30-60 minutos'
                    ]
                });
                ...
                reglas.push({
                    ...
                    tipo_riesgo: 'estable',
                    valor: temperatura,
                    mensaje: `Temperatura ligeramente alta (${temperatura.toFixed(1)} °C). Nivel bajo de alerta preventiva.`,
                    recomendacion: 'Por ahora está controlable. Haz un ajuste suave de ventilación para no subir a riesgo medio.',
                    acciones: [
                        'Ajustar ventilación de forma suave',
                        'Evitar riegos en horas de mayor radiación',
                ...
                reglas.push({
                    ...
                    tipo_riesgo: 'critico',
                    valor: temperatura,
                    mensaje: `Temperatura baja (${temperatura.toFixed(1)} °C). Puede reducirse el desarrollo del cultivo.`,
                    recomendacion: 'Está fresco para el cultivo. Ayuda mucho reducir corrientes y conservar calor durante la noche.',
                    acciones: [
                        'Reducir ventilación nocturna',
                        'Revisar sellado de puertas/cortinas',
                        'Programar riego en horas más templadas'
                    ]
                });
                ...
                reglas.push({
                    ...
                    tipo_riesgo: 'pre_critico',
                    valor: temperatura,
                    mensaje: `Temperatura en nivel medio-bajo (${temperatura.toFixed(1)} °C).`,
                    recomendacion: 'Se está enfriando más de lo ideal. Te sugiero reducir corrientes para no afectar el desarrollo.',
                    acciones: [
                        'Reducir ventilación durante la noche',
                        'Revisar puntos fríos del invernadero',
                        'Controlar nueva lectura en la próxima hora'
                    ]
                });
                ...
                reglas.push({
                    ...
                    tipo_riesgo: 'estable',
                    valor: temperatura,
                    mensaje: `Temperatura ligeramente baja (${temperatura.toFixed(1)} °C). Nivel bajo de alerta preventiva.`,
                    recomendacion: 'No es grave, pero conviene cuidar el calor acumulado para mantener crecimiento estable.',
                    acciones: [
                        'Evitar pérdidas de calor al final del día',
                        'Ajustar ventilación mínima',
            ...
            if (humedad >= 92) {
                reglas.push({
                    ...
                    tipo_riesgo: 'critico',
                    valor: humedad,
                    mensaje: `Humedad relativa alta (${humedad.toFixed(1)} %). Riesgo de hongos y enfermedades.`,
                    recomendacion: 'La humedad está bastante alta. Te recomiendo mover más aire para evitar problemas de hongos.',
                    acciones: [
                        'Incrementar ventilación y circulación de aire',
                        'Evitar riegos tardíos al final del día',
                        'Monitorear hojas con signos tempranos de enfermedad'
                    ]
                });
            } else if (humedad >= 85) {
                reglas.push({
                    ...
                    tipo_riesgo: 'pre_critico',
                    valor: humedad,
                    mensaje: `Humedad en nivel medio-alto (${humedad.toFixed(1)} %).`,
                    recomendacion: 'La humedad está subiendo. Si ventilas un poco más ahora, evitas que se vuelva crítica.',
                    acciones: [
                        'Aumentar circulación de aire',
                        'Evitar riego tardío',
                        'Revisar condensación en hojas'
                    ]
                });
            } /* else if (humedad >= 80) {
                reglas.push({
                    ...
                    tipo_riesgo: 'estable',
                    valor: humedad,
                    mensaje: `Humedad ligeramente alta (${humedad.toFixed(1)} %). Nivel bajo de alerta preventiva.`,
                    recomendacion: 'Vas bien, solo mantén ventilación equilibrada para que no escale a nivel medio.',
                    acciones: [
                        'Ventilar de manera gradual',
                        'Evitar acumulación de humedad nocturna',
                        'Seguir monitoreo'
                    ]
                });
            } */ else if (humedad <= 30) {
                reglas.push({
                    ...
                    tipo_riesgo: 'critico',
                    valor: humedad,
                    mensaje: `Humedad relativa baja (${humedad.toFixed(1)} %). Puede aumentar estrés hídrico.`,
                    recomendacion: 'El ambiente está seco. Podrías hacer ajustes de riego y bajar ventilación en picos de calor.',
                    acciones: [
                        'Aplicar riego fraccionado',
                        'Reducir ventilación en las horas más secas',
                ...
                reglas.push({
                    ...
                    tipo_riesgo: 'pre_critico',
                    valor: humedad,
                    mensaje: `Humedad en nivel medio-bajo (${humedad.toFixed(1)} %).`,
                    recomendacion: 'La humedad está bajando más de la cuenta; conviene ajustar riego y ventilación fina.',
                    acciones: [
                        'Aplicar riego fraccionado',
                        'Reducir ventilación en horas secas',
                        'Verificar uniformidad del sistema'
                    ]
                });
            }/*  else if (humedad <= 50) {
                reglas.push({
                    ...
                    tipo_riesgo: 'estable',
                    valor: humedad,
                    mensaje: `Humedad ligeramente baja (${humedad.toFixed(1)} %). Nivel bajo de alerta preventiva.`,
                    recomendacion: 'Todavía es manejable, pero te sirve vigilar para mantener confort hídrico en la planta.',
                    acciones: [
                        'Monitorear evolución en la tarde',
            ...
            if (dpv >= 2.2) {
                reglas.push({
                    ...
                    tipo_riesgo: 'critico',
                    valor: dpv,
                    mensaje: `DPV elevado (${dpv.toFixed(2)} kPa). La planta transpira más de lo ideal.`,
                    recomendacion: 'Tu DPV está alto: conviene subir humedad ambiental o bajar temperatura para evitar estrés hídrico.',
                    acciones: [
                        'Aumentar humedad del ambiente de forma gradual',
                        'Programar pulsos de riego más frecuentes',
                        'Reducir exposición a calor extremo en horas pico'
                    ]
                });
            } else if (dpv >= 1.6) {
                reglas.push({
                    ...
                    tipo_riesgo: 'pre_critico',
                    valor: dpv,
                    mensaje: `DPV en nivel medio (${dpv.toFixed(2)} kPa).`,
                    recomendacion: 'El DPV está subiendo; te recomiendo un ajuste preventivo para no llegar a estrés fuerte.',
                    acciones: [
                        'Subir humedad de forma gradual',
                ...
                reglas.push({
                    ...
                    tipo_riesgo: 'estable',
                    valor: dpv,
                    mensaje: `DPV ligeramente alto (${dpv.toFixed(2)} kPa). Nivel bajo de alerta preventiva.`,
                    recomendacion: 'Aún no es crítico; con pequeños ajustes de clima puedes mantenerlo en zona cómoda.',
                    acciones: [
                        'Revisar ventilación y humedad',
                        'Monitorear siguiente lectura',
                        'Corregir temprano para evitar escalamiento'
                    ]
                });
            } */ else if (dpv <= 0.25) {
                reglas.push({
                    ...
                    tipo_riesgo: 'critico',
                    valor: dpv,
                    mensaje: `DPV muy bajo (${dpv.toFixed(2)} kPa). Riesgo de condensación y baja transpiración.`,
                    recomendacion: 'El DPV está demasiado bajo. Te puede ayudar ventilar un poco para evitar condensación en hojas.',
                    acciones: [
                        'Mejorar ventilación para secar el microclima',
                        'Evitar exceso de riego en periodos fríos',
                        'Inspeccionar presencia de condensación en hojas'
                    ]
                });
            } else if (dpv <= 0.4) {
                reglas.push({
                    ...
                    tipo_riesgo: 'pre_critico',
                    valor: dpv,
                    mensaje: `DPV en nivel medio-bajo (${dpv.toFixed(2)} kPa).`,
                    recomendacion: 'El DPV está bajo y podría afectar transpiración; una ventilación suave suele ayudar.',
                    acciones: [
                        'Ventilar gradualmente',
                        'Evitar exceso de humedad sostenida',
                        'Verificar condensación en el cultivo'
                    ]
                });
            } /* else if (dpv <= 0.6) {
                reglas.push({
                    ...
                    tipo_riesgo: 'estable',
                    valor: dpv,
                    mensaje: `DPV ligeramente bajo (${dpv.toFixed(2)} kPa). Nivel bajo de alerta preventiva.`,
                    recomendacion: 'Todo bajo control, solo mantén vigilancia para sostener un intercambio de vapor adecuado.',
                    acciones: [
                        'Monitorear tendencia próxima',
                        'Mantener ventilación estable',
                        'Evitar sobre-riego'
                    ]
                });
            } */
        ...
        for (const regla of reglas) {
            const existe = await db.oneOrNone(
                `SELECT id_alerta
                 ...
                 WHERE id_lote = $1
                   AND id_usuario = $2
                   AND estado IN ('generada', 'enviada', 'vista')
                   AND tipo_riesgo = $3
                   AND mensaje_texto ILIKE $4
                   AND fecha_generacion > NOW() - INTERVAL '6 hour'
                 LIMIT 1`,
            ...
            const nuevaAlerta = await db.one(
                `INSERT INTO alertas (
                    id_lote, id_usuario, tipo_riesgo, mensaje_texto, dpv_desencadenante, estado
                ) VALUES ($1, $2, $3, $4, $5, 'generada')
                RETURNING id_alerta`,
                [
                    id_lote,
                    idUsuario,
                    regla.tipo_riesgo,
                    `[${regla.clave.toUpperCase()}] ${regla.mensaje}`,
                    regla.clave === 'dpv' ? regla.valor : null
                ]
            );
            ...
            await db.query(
                ...
                [
                    nuevaAlerta.id_alerta,
                    id_lote,
                    regla.tipo_riesgo === 'critico' ? 'alto' : (regla.tipo_riesgo === 'pre_critico' ? 'medio' : 'bajo'),
                    regla.recomendacion,
        ...
        res.status(500).json({
            error: 'Error al obtener alertas por cultivo',
            code: 'ALERTAS_CULTIVO_ERROR'
        });
    }
}
