 ...
 * POST /api/v1/lotes
 */
async function crearLote(req: CustomRequest, res: Response): Promise<void> {
 ...
 * GET /api/v1/lotes
 */
async function obtenerLotes(req: CustomRequest, res: Response): Promise<void> {
    const idUsuario = req.usuario?.id_usuario;
 ...
 * GET /api/v1/lotes/:id
 */
async function obtenerLote(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    const idUsuario = req.usuario?.id_usuario;
 ...
 * PUT /api/v1/lotes/:id
 */
async function actualizarLote(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    ...
    try {
        ...
        const loteExistente = await db.oneOrNone(
 ...
 */
async function eliminarLote(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    const idUsuario = req.usuario?.id_usuario;
...
/**
 * Obtener lecturas de un lote
 * GET /api/v1/lotes/:id/lecturas
 */
async function obtenerLecturasLote(req: CustomRequest, res: Response): Promise<void> {
    const { id } = req.params;
    const idUsuario = req.usuario?.id_usuario;
    const { horas = 24, limit = 1000 } = req.query;
    ...
    try {
        ...
        // Obtener todas las lecturas del lote
        const lecturas = await db.query(
            `SELECT id_lectura, id_sensor, timestamp, temperatura_celsius, humedad_relativa,
             ...
             WHERE id_lote = $1 AND timestamp > NOW() - INTERVAL '1 hour' * $2
             ORDER BY timestamp DESC
             LIMIT $3`,
            [id, horas, limit]
        );

        // logger.info(`Lecturas obtenidas para lote ${id}: ${lecturas.length} ${lecturas}`);

        res.status(200).json({
            success: true,
            total: lecturas.length,
            lote_id: id,
            horas: parseInt(horas as string),
            data: lecturas
        });

        logger.info(`Lecturas obtenidas para lote: ${id}`);
    } catch (error) {
        logger.error('Error al obtener lecturas del lote:', error);
        res.status(500).json({
            error: 'Error al obtener lecturas del lote'
        });
    }
}
