...
async function populateDatabase() {
    try {
        ...
        const alertaResult = await client.query(`
            ...
            VALUES 
            ($1, $2, 'estrés_hídrico', 1.45, 'Tendencia de estrés hídrico detectada', 'activa'),
            ($3, $2, 'temperatura_baja', 0.5, 'Temperatura por debajo de lo óptimo', 'resuelta'),
            ($5, $2, 'humedad_alta', 0.3, 'Humedad relativa elevada', 'resuelta')
            RETURNING id_alerta;
        `, [loteIds[0], userId, loteIds[1], userId, loteIds[2], userId]);
        ...
        const recomResult = await client.query(`
            ...
            VALUES 
            ($1, $2, 'alta', 'Aumentar frecuencia de riego a cada 4 horas'),
            ($3, $4, 'media', 'Abrir ventilaciones laterales 30 minutos'),
            ($5, $6, 'baja', 'Reducir frecuencia de riego')
        ...
        console.error('❌ Error durante población:', err.message);
        process.exit(1);
    }
}
